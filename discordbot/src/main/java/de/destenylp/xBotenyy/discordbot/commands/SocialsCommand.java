...
public class SocialsCommand extends AbstractGuildCommand {
    ...
    private final String defaultTiktokMessage;
    ...
    public SocialsCommand(SocialService service, String defaultYoutubeMessage, String defaultTwitchMessage,
        ...
        this.defaultTiktokMessage = defaultTiktokMessage;
    ...
    public CommandData getCommandData() {
        ...
        return Commands.slash("socials", "Verwalte YouTube-, Twitch- und TikTok-Benachrichtigungen")
                ...
                .addSubcommands(
                        new SubcommandData("add", "Fuegt einen neuen Account hinzu, der ueberwacht werden soll")
                                ...
                                .addOption(OptionType.STRING, "tiktok", "TikTok Benutzername", false),
                        new SubcommandData("edit", "Bearbeitet einen bestehenden Account")
                                ...
                                .addOption(OptionType.STRING, "tiktok", "Neuer TikTok Benutzername ('none' zum Entfernen)", false),
    ...
    private void handleAdd(SlashCommandInteractionEvent event, Guild guild) {
        ...
        OptionMapping tiktokOption = event.getOption("tiktok");
        if (youtubeOption == null && twitchOption == null && tiktokOption == null) {
            event.reply("Bitte gib mindestens einen YouTube-Kanal, Twitch-Benutzernamen oder TikTok-Benutzernamen an.")
        ...
        String tiktokUsername = tiktokOption != null ? tiktokOption.getAsString() : null;
        if (tiktokUsername != null && !SocialValidators.isValidTiktokUsername(tiktokUsername)) {
            event.reply("Der TikTok Benutzername ist ungueltig.").setEphemeral(true).queue();
            return;
        }
        SocialAccount draft = SocialAccount.builder()
                ...
                .tiktokUsername(tiktokUsername != null ? tiktokUsername.toLowerCase() : null)
                .tiktokMessage(defaultTiktokMessage)
                .build();
    ...
    private void handleEdit(SlashCommandInteractionEvent event, Guild guild) {
        ...
        if (tiktokOption != null && !isClearValue(tiktokOption.getAsString())
                && !SocialValidators.isValidTiktokUsername(tiktokOption.getAsString())) {
            event.reply("Der TikTok Benutzername ist ungueltig.").setEphemeral(true).queue();
            return;
        }
        ...
        boolean finalHasTiktok = tiktokOption != null ? !isClearValue(tiktokOption.getAsString()) : account.hasTiktok();
        if (!finalHasYoutube && !finalHasTwitch && !finalHasTiktok) {
        ...
        if (tiktokOption != null) {
            if (isClearValue(tiktokOption.getAsString())) {
                account.clearTiktok();
            } else {
                account.setTiktokUsername(tiktokOption.getAsString().toLowerCase());
            }
        }
    ...
    private void handleMessage(SlashCommandInteractionEvent event, Guild guild) {
        ...
        if (platform.equals("tiktok") && !account.hasTiktok()) {
            event.reply("Fuer diesen Account ist kein TikTok-Account konfiguriert.").setEphemeral(true).queue();
            return;
        }
        ...
        var template = switch (platform) {
            case "youtube" -> account.getYoutubeTemplate();
            case "twitch" -> account.getTwitchTemplate();
            default -> account.getTiktokTemplate();
        };
    ...
    private void handleList(SlashCommandInteractionEvent event, Guild guild) {
        ...
        for (SocialAccount account : accounts) {
            String value = "Kanal: <#" + account.getChannelId() + ">\n" +
                    ...
                    "TikTok: " + (account.hasTiktok() ? "`" + account.getTiktokUsername() + "`" : "Nicht konfiguriert");
    ...
    private void handleTest(SlashCommandInteractionEvent event, Guild guild) {
        ...
        if (platform.equals("youtube")) {
        ...
        } else if (platform.equals("twitch")) {
            ...
            event.deferReply(true).queue();
            String tiktokUsername = account.getTiktokUsername();
            CompletableFuture.supplyAsync(() -> tiktokFeedClient.fetchLatestVideo(tiktokUsername))
                    .thenAccept(videoOpt -> {
                        if (videoOpt.isEmpty()) {
                            event.getHook().sendMessage("Es konnten keine echten Video-Daten fuer diesen Account abgerufen werden "
                                    + "(kein Video gefunden oder Fehler beim Abruf). Nutze `/socials status` fuer Details.").queue();
                            return;
                        }
                        RenderedMessage built = SocialMessageFactory.buildTiktokMessage(account, videoOpt.get(), guild);
                        sendPreview(event, platform, id, built);
                    })
                    .exceptionally(ex -> {
                        LOGGER.warn("Error fetching real TikTok data for test preview: {}", ex.getMessage());
                        event.getHook().sendMessage("Es ist ein Fehler beim Abrufen der echten Video-Daten aufgetreten.").queue();
                        return null;
                    });
        }
    ...
    private void handleStatus(SlashCommandInteractionEvent event, Guild guild) {
        ...
        eb.addField("TikTok Intervall", SocialsPollStatus.getTiktokIntervalMinutes() + " min", true);
        ...
        eb.addField("Letzter TikTok-Check", formatRelative(SocialsPollStatus.getLastTiktokPoll()), true);
        String youtubeError = SocialsPollStatus.getLastYoutubeError();
        String twitchError = SocialsPollStatus.getLastTwitchError();
        String tiktokError = SocialsPollStatus.getLastTiktokError();
        ...
        if (tiktokError != null) {
            eb.addField("Letzter TikTok-Fehler", tiktokError, false);
        }
        List<SocialAccount> accounts = service.getAccounts(guild.getId());
        if (accounts.isEmpty()) {
            ...
            for (SocialAccount account : accounts) {
                ...
                if (account.hasTwitch()) {
                    value.append("Twitch: `").append(account.getTwitchLogin()).append("`, aktuell live: ")
                            .append(account.isTwitchCurrentlyLive() ? "Ja" : "Nein").append("\n");
                }
                if (account.hasTiktok()) {
                    value.append("TikTok: `").append(account.getTiktokUsername()).append("`, letztes Video: ")
                            .append(account.getLastTiktokVideoId() != null
                                    ? "`" + account.getLastTiktokVideoId() + "`" : "noch keine Baseline");
                }
